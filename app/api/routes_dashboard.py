...
def register_templates(templates: Jinja2Templates) -> None:
    ...
    def dashboard(request: Request, user=Depends(current_user), db: Session = Depends(get_db)) -> HTMLResponse:
        ...
        filled_slots = min(16, int(provider_count) + int(docs_count) + len(recent))
        inventory_slots = [{"index": idx, "filled": idx < filled_slots} for idx in range(16)]
    ...
    def add_appointment(
        ...
        invoice_cents = estimated_invoice_cents if estimated_invoice_cents is not None and estimated_invoice_cents >= 0 else provider.estimated_copay_cents
